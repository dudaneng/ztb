...
class GoodsController extends BaseFrontController
{
	...
	**/
    ...
    public function actionCart(){
		...
		if($user_id){
			...
			return $this->render('cart', ['row' => $data, 'time'=>$time, 'price'=>$price]);
		}else{
	...
	**/
    ...
    public function actionDetail(){
		...
		return $this->render('detail', ['row'=>$goods]);
	}
	/******************** 1 ********************/
	...
	**/
    ...
    public function actionCartadd(){
		...
		$user_id = dy::getLoginOne('user_id');
		if($user_id){
			...
			if(!$row){
				...
				$list = dy::execAdd('cart', $data);
			}else{
				$data = array(
					'create_time' => time(),
					'update_time' => time(),
					'goods_number' => $row['goods_number'] + 1,
				);
				$list = dy::execUpdate('cart', $data, $row['id']);
			}
			echo 1;
		}else{
			echo 0;
	...
	**/
    ...
    public function actionChangenumbers(){
		...
		$price1 = Yii::$app->request->post('price1');
		$allprice = Yii::$app->request->post('allprice');
		
		$row = Cart::find()->where(['id' => $id])->one();
		...
		$price = $list['goods_number'] * $goods_list['price_vip']; 
		
		$all = $allprice - $price1 + $price;
		
		echo json_encode(array('number'=>$list['goods_number'], 'price'=>$price, 'all' => $all));
	}
	/******************** 1 ********************/
	...
	**/
    ...
    public function actionChangedetail(){
		...
		$row = Cart::find()->where(['goods_id' => $id, 'user_id'=>$user_id])->one();
		...
		if($user_id){
			if($row){
				...
				dy::execUpdate('cart', $data, $id, 'goods_id');
				
				$row_cart = Cart::find()->where(['user_id'=>$user_id])->all();
				
				$counts = '';
				foreach($row_cart as $v){
					$counts += $v['goods_number'];
				}
	...
	**/
    ...
    public function actionOrder(){
		...
		if($user_id){
			$session = Yii::$app->session->set('act_id', '');//清空活动id
	...
	**/
    ...
    public function actionPaysub(){
		...
		if($goods_list['price_discount'] == 0){
			if($id){
