...
class LoginController extends BaseFrontController
{
	...
	**/
    ...
    public function actionLogin(){
		...
		$openid = '';
		if(Yii::$app->request->cookies->get('openid') != ''){
			$openid = Yii::$app->request->cookies->get('openid')->value;//获取微信openid
			$openid = wx::decrypt($openid,"jdf92jkdfo30HHH899Lks");
		}
		...
		if($list){
			if($list['status'] == 1){
				...
				if($openid){
					dy::execUpdate('user', array('last_time'=>time(), 'openid'=>$openid), $list['id']);
				}else{
	...
	/**
	 * 20150701 add by dudanfeng 1 退出登陆
	**/
	/******************** 1 ********************/
	...
	/**
	 * 20150701 add by dudanfeng 1 用户注册
	**/
	/******************** 1 ********************/
	...
	/**
	 * 20150701 add by dudanfeng 1 用户注册 数据提交
	**/
	/******************** 1 ********************/
