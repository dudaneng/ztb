...
class PayController extends BaseFrontController
{
	...
	**/
	...
	{
		require_once("/components/alipay/alipayconfig.php");
		...
		if(!$verify_result) {//验证成功
			...
			$out_trade_no = $_POST['out_trade_no'] = 'E7154843177876';
			...
			if($_POST['trade_status'] == 'TRADE_FINISHED') {
				...
				$order = Order::find()->where(['order_sn'=>$out_trade_no])->one();//获取订单信息
				...
				if($order['price_all'] == $order['price_discount']){
					...
					if($activity){
						...
						$ac_numbers = ActNumber::find()->andWhere(['act_id'=>$act_id[$key], 'user_id'=>$order['user_id']])->count();//此活动已参加次数
						
						if($ac_numbers < $numbers[$key]){//使用次数少于规定次数
							dy::execAdd('act_number', array('user_id'=>$order['user_id'], 'act_id'=>$act_id[$key], 'numbers'=>1, 'status'=>1));
						}
					}
				}
				$user = User::find()->where(['id'=>$order['user_id']])->one();//获取用户信息
				
				if($user['act_status'] == 0){
					dy::execStatusUp('user', array('act_status'=>1), $order['user_id']);
				}
				...
				echo "<script src='http://122.114.62.13:8080/socket.io/socket.io.js'></script>

					<script>
					window.onload =function(){
						var iosocket = io.connect('http://122.114.62.13:8080');
						
						var msg = $out_trade_no;
						iosocket.send(msg);
					}
					</script>";
				/**
	...
	public function actionReturn(){
		...
		if($verify_result) {//验证成功
			...
			if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
				...
				$result = dy::sendCode ( $target = "http://106.ihuyi.cn/webservice/sms.php?method=Submit",$sms_account, $sms_password, $phone, $code);
				**/
				return $this->redirect(['paysuccess', 'order_sn'=>$out_trade_no]);
			}
			else {
