...
class PersonController extends BaseFrontController
{
	...
	**/
    ...
    public function actionIndex(){
		...
		if($user_id){
			...
			$this->counts = dy::getOrdersNumbers();//订单数量统计
			$this->menu = dy::getAreaSelect();//获取区域
			$this->numbers = dy::getCartNumbers();//获取数量
			$this->setting = dy::getSetting();//版权信息
			
			return $this->render('index', ['row'=>$data, 'page'=>$pageList, 'type'=>$type, 'time_set'=>$time_set]);
		}else{
	...
	**/
	...
	public function actionForbid(){
		...
		if($type == 1){
			...
			$rst = dy::execDelete('order', $id);//改变状态
		}else{
			$rst = '';
		}
	...
	**/
    ...
    public function actionActivity(){
		...
		$command = $connection->createCommand("SELECT a.*,b.*,b.id as activity_id FROM {{%act_number}} as a left join {{%activity}} as b on b.id=a.act_id WHERE a.user_id=$user_id and a.status=:a and a.numbers > 0;");
